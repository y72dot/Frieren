"""LLM agent orchestration service, independent from plugin discovery."""
...
class LlmAgentService:
    """Run one LLM trigger through session, runtime, and AgentLoop services."""

    async def handle_trigger(self, payload: dict[str, Any], bot) -> bool:
        return await _handle_trigger(payload, bot)


default_agent_service = LlmAgentService()


async def handle_trigger(payload: dict[str, Any], bot) -> bool:
    """Compatibility function around the default service instance."""
    return await default_agent_service.handle_trigger(payload, bot)


async def _handle_trigger(payload: dict[str, Any], bot) -> bool:
    """Run a validated ``llm_type=trigger`` payload through the agent loop."""
    ...
    if agent_loop is None:
        raise RuntimeError("LLM AgentLoop was not initialized")
...
def _inject_recent_history(messages: list[dict], session_key: str, bot, payload: dict) -> None:
    ...
    bot_qq = bot.config.bot.qq
    lines = [format_message(m, bot_qq=bot_qq, include_time=True) for m in msgs]
