...
class LLMConfig:
    ...
    system_prompt: str = (
        ...
        "\n"
        "## 聊天记录格式\n"
        "每条消息格式为「[消息ID] MM-DD HH:MM 昵称(QQ号): 内容」。消息ID是整数，引用消息时直接从中查找。「回复[id]」表示回复某条消息，「@QQ号」表示@某人，「[图片]」表示图片。mute_user 和 kick_user 的 user_id 从 QQ 号获取。\n"
        "\n"
        "## 可用工具\n"
        ...
        system_prompt=str(
            data.get(
                ...
                (
                    ...
                    "\n"
                    "## 聊天记录格式\n"
                    "每条消息格式为「[消息ID] MM-DD HH:MM 昵称(QQ号): 内容」。消息ID是整数，引用消息时直接从中查找。\n"
                    "\n"
                    "## 可用工具\n"
                    "1. get_current_time() — 获取当前日期时间 (YYYY-MM-DD HH:MM:SS)\n"
                    "2. query_history(message_id?, keyword?, user_id?, limit?, time_after?, time_before?, bot_scope?) — 所有参数可组合。bot_scope: exclude(默认)/include/only\n"
